...
> specshub awareness), cross-harness via plugin or `npx skills`.

[![npm](https://img.shields.io/npm/v/specshub?style=flat&logo=npm)](https://www.npmjs.com/package/specshub)
...
Whether you call it `.specshub/`, `internal-docs/`, `docs/agent/`, or
...
- **8 bundled skills** that AI agents use directly: the **specshub awareness
  skill** (auto-invoked when an agent enters a specshub repo) + **7 SDD
...
- **`specshub` knows how to work with itself** — the awareness skill teaches
...
cd ~ && npx skills add github:Sumit1993/specshub --global

# In a code repo: scenario 1 — docs live in this repo
cd ~/code/my-api
npx specshub init --in-repo

# Or: scenario 2 — create a sibling external hub
npx specshub init --external --name my-team-specshub --private

# Or: scenario 4 (hybrid) — start in-repo, later register with an existing hub
npx specshub init --in-repo
npx specshub link ~/code/some-existing-hub
```

That's it. Open Claude Code / Cursor / Codex / your favorite agent in the code
repo and start using `/specify`, `/plan`, etc. The `specshub` skill loads
automatically when you're in a specshub-aware repo.

To remove the skills later: `cd ~ && npx skills remove github:Sumit1993/specshub --global`.

## The three scenarios

| Scenario | When to use it | What `init` creates |
|----------|----------------|---------------------|
| **In-repo** | Single-repo project. OSS where the constitution and specs are part of the open source story. Personal projects. Matches the spec-kit / BMad / Kiro convention. | `<code-repo>/.specshub/` (real dir, committed to your code repo) |
| **External** | Multi-repo portfolio. Strategy or pre-launch material that shouldn't be in a public code repo. One hub serves N projects with cross-cutting MAP and cross-refs. | A new sibling hub repo (default name: `<code-repo>-specshub`, visibility your choice) with first project registered |
...
| `link <hub-path>` | Add this code repo to an existing hub (auto-detects storage: in-repo if `.specshub/` has content, hub-owned if empty) |
| `unlink [--hub <path>]` | Remove a specshub linkage (updates both metadata files; suggests commits in both) |
...
`npx skills add github:Sumit1993/specshub` and `npx skills remove
github:Sumit1993/specshub` directly.

Run `npx specshub <command> --help` for per-command flags.

## How `link` works

Auto-detects the storage mode based on whether your code repo's `.specshub/`
already has content:

```bash
# Case A — empty/missing .specshub/ → storage="hub-owned" (scenario 2)
cd ~/code/my-web
npx specshub link ~/code/my-team-specshub
# Hub gets a new projects/my-web/.specshub/ stub directory
# Code repo gets mode=external metadata pointing at the hub

# Case B — populated .specshub/ → storage="in-repo" (scenario 4 / hybrid)
...
npx specshub link <hub> --storage hub-owned
npx specshub link <hub> --storage in-repo
```
...
- `<code-repo>/.specshub/metadata.json` — small file declaring mode, project
  name, optional hub references. Always tracked in your code repo's git.
  **The only file specshub writes to your code repo.**
- `<code-repo>/.specshub/.specify/`, `specs/`, `decisions/` — for in-repo
  and hybrid modes only. Created on demand by the SDD skills (`/specify`,
  `/constitution`, etc.), not at init time.

specshub does **not** touch `AGENTS.md`, `CLAUDE.md`, or any other
agent-facing files in your code repo. The `specshub` awareness skill (loaded
once per machine via `npx skills add`) auto-invokes when it sees
`.specshub/metadata.json` and reads the per-repo facts directly from there
— project name, mode, docs root, commit-hygiene rules. See
[ADR 0005](.specshub/decisions/0005-drop-per-repo-preamble.md).
...
- `<hub>/projects/<project>/.specshub/` — for hub-owned projects only,
...
**specshub never auto-commits.** It always *suggests* the exact `git` commands
in chat output and lets you run them. The `specshub` awareness skill teaches
agents the same rule.

For external mode, you commit to two repos:
- **Code repo** for everything outside `.specshub/` (your code) plus
  `.specshub/metadata.json` itself (the link pointer)
...
|-------|:------------:|---------|
| `specshub` | yes | Teaches the agent: detection, path resolution, read order, commit hygiene, proactive doc-update suggestions. Loads when `.specshub/metadata.json` is present. |
...
- Network access for `npx skills add github:Sumit1993/specshub`. Offline users
  can pre-cache the package or install `vercel-labs/skills` globally.

## Cross-platform notes

specshub doesn't use symlinks (see
[ADR 0001](.specshub/decisions/0001-no-symlinks.md)) so the cross-platform story is
...
```ts
import { init, link, verify } from "specshub";
...
The architecture was nailed down through a multi-session grilling. specshub
dogfoods its own convention: its design docs live at
[`.specshub/`](.specshub/) using in-repo mode.

- **Architecture**: [`.specshub/ARCHITECTURE.md`](.specshub/ARCHITECTURE.md)
...
- **ADRs** for the load-bearing architectural decisions:
  - [0001 — no symlinks](.specshub/decisions/0001-no-symlinks.md)
  - [0002 — hand-authored skills, no scheduled sync](.specshub/decisions/0002-hand-authored-skills.md)
  - [0003 — delegate skill install to vercel-labs/skills](.specshub/decisions/0003-delegate-install-to-vercel-labs-skills.md)
  - [0004 — remove the user-level teacher breadcrumb](.specshub/decisions/0004-remove-teacher-breadcrumb.md)
...
- **[vercel-labs/skills](https://github.com/vercel-labs/skills)** (Apache 2.0)
  — the cross-harness skill installer. specshub delegates skill install to
  `npx skills add github:Sumit1993/specshub`. See
