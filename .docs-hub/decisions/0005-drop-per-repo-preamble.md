...
1. The auto-invoked `docs-hub` awareness skill (loads when
   `.docs-hub/metadata.json` is present in the cwd or a parent).
...
agent without prior docs-hub awareness benefits from a marker in AGENTS.md.
...
Re-examining ADR 0004's keep argument: the "cold-start agent without docs-hub
...
actionable — they still need to read and interpret `.docs-hub/metadata.json`
...
they write to a code repo is `.docs-hub/metadata.json`.

The `docs-hub` awareness skill is the sole runtime mechanism for teaching
agents how to operate inside a docs-hub repo. Detection signal:
`.docs-hub/metadata.json` exists. Data source: same file.
...
2. **Write-time invasion is hostile.** Most dev tools refuse to mutate
   user-authored files. docs-hub was writing into AGENTS.md and CLAUDE.md —
   files the user owns and may have hand-tuned. Dropping the preamble means
   docs-hub now touches only its own dotdir.
...
4. **Cleaner uninstall.** `unlink` removes the linkage from both
   metadata files. To remove docs-hub from a repo entirely, the user
   deletes `.docs-hub/metadata.json` themselves — there is nothing else
   docs-hub left behind.
...
5. **The "cold-start agent" case is weaker than ADR 0004 estimated.** Without
   ...
   docs-hub-specific without the skill. With the skill, the preamble is
   redundant.

6. **The `.docs-hub/` directory IS the orientation breadcrumb.** Any agent
   doing a directory scan or `ls` sees `.docs-hub/`. Any agent reading
   `.docs-hub/metadata.json` immediately has structured info on mode +
...
- **Suggested commit commands shrink.** Was `git add .docs-hub AGENTS.md
  CLAUDE.md`; now `git add .docs-hub`.
...
3. **Replace the preamble with a `.docs-hub/README.md` autocreated on init.**
...
- [ARCHITECTURE.md](../ARCHITECTURE.md) — updated to reflect single-mechanism
  awareness
- [`plugin/skills/docs-hub/SKILL.md`](../../plugin/skills/docs-hub/SKILL.md) —
