...
(`~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, etc.). The block said: "specshub
is installed; here's how to interpret `.specshub/metadata.json` artifacts
when you encounter them."

During the v0.0.1 grilling, an early plan ("G2") proposed keeping the
breadcrumb as a *pointer* to the new richer `specshub` SKILL.md awareness
...
1. **The `specshub` awareness skill** (`plugin/skills/specshub/SKILL.md`) —
   auto-invoked when an agent sees `.specshub/metadata.json` in a repo.
...
4. **One mechanism.** "How does an agent learn about specshub? The specshub
...
- **Edge case left uncovered:** users on harnesses without skill
  ...
  specshub. None of the agents in the vercel-labs/skills support matrix
...
2. **Convert the breadcrumb into its own small skill.** Rejected: the
   breadcrumb's content is "see the specshub skill for details" — making a
   skill whose body is "see another skill" is noise. The `specshub` skill
...
3. **Remove the breadcrumb but ALSO drop the per-repo AGENTS.md preamble.**
   ...
   preamble. An agent opening a repo without prior specshub awareness should
